"""
run_funnel_compare.py — Compare PEACHES h-side, ChEES, and NUTS on Neal's funnel.

All use find_init_step_size=False with conservative initial eps, same n_samp/warmup.
"""
...
if __name__ == "__main__":
    ...
    results["PEACHES h-side"] = (s, info)

    # ────────── PEACHES h-walk (no find) ──────────
    print("\n--- PEACHES h-walk (find_init_step_size=False, eps=0.01) ---")
    s, info = peaches.sampler_peaches(
        log_prob, init, n_samp, warmup=warmup,
        move="h-walk", step_size=0.01, find_init_step_size=False, seed=seed)
    results["PEACHES h-walk"] = (s, info)

    # ────────── ChEES (no find) ──────────
    print("\n--- ChEES (find_init_step_size=False, eps=0.01) ---")
    s, info = chess.sampler_chess(
        log_prob, init, n_samp, warmup=warmup,
        step_size=0.01, find_init_step_size=False, seed=seed)
    results["ChEES"] = (s, info)

    # # ────────── ChEES (find) ──────────
    # print("\n--- ChEES (find_init_step_size=True, eps=0.1) ---")
    # s, info = chess.sampler_chess(
    #     log_prob, init, n_samp, warmup=warmup,
    #     step_size=0.1, find_init_step_size=True, seed=seed)
    # results["ChEES (find)"] = (s, info)

    # # ────────── NUTS (no find) ──────────
    # print("\n--- NUTS (find_init_step_size=False, eps=0.01) ---")
    # s, info = nuts.sampler_nuts(
    #     log_prob, init, n_samp, warmup=warmup,
    #     step_size=0.01, find_init_step_size=False, seed=seed)
    # results["NUTS"] = (s, info)

    # # ────────── NUTS (find) ──────────
    # print("\n--- NUTS (find_init_step_size=True, eps=0.1) ---")
    # s, info = nuts.sampler_nuts(
    #     log_prob, init, n_samp, warmup=warmup,
    #     step_size=0.1, find_init_step_size=True, seed=seed)
    # results["NUTS (find)"] = (s, info)

    # ══════════════════════════════════════════════════════════════════════════
    # Report
    # ══════════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 80)
    print(f"{'Sampler':25s}  {'Accept':>8s}  {'v mean':>8s}  {'v var':>8s}  {'x var':>8s}  {'eps':>10s}")
    print("-" * 80)
    for name, (samps, info) in results.items():
        ...
        eps = info.get("final_step_size", info.get("final_eps", 0))
        print(f"{name:25s}  {acc:8.3f}  {np.mean(v):8.3f}  {np.var(v):8.3f}  {x_var:8.1f}  {eps:10.4f}")
    print("=" * 80)
    ...
    plt.tight_layout()
    plt.savefig("funnel_compare.png", dpi=150)
    plt.show()
    print("\nSaved funnel_compare.png")